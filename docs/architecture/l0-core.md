# L0 — Rust Core Architecture

> **Role in OpenObscure:** The Rust core is the **hard enforcement** layer — the same detection engines, FPE encryption, image pipeline, and cognitive firewall serve both deployment models. In the **Gateway model** it runs as an HTTP reverse proxy sitting between the AI agent and LLM providers. In the **Embedded model** it compiles as a native library (`.a` / `.so`) called directly from Swift or Kotlin via UniFFI bindings — no HTTP server, no sockets. For the full system context, see [System Overview](system-overview.md).
...
**Embedded Model** — the same Rust core compiles as a native library. The host app calls it directly; no proxy process or HTTP server is involved.
...
flowchart LR
    subgraph Device [" Mobile Device "]
        ...
        subgraph App [" Host App "]
            direction TB
            A["<b>App Code</b><br/>(Swift / Kotlin)"]
            B["<b>UniFFI Bindings</b><br/>(auto-generated)"]
            A --> B
        end

        subgraph Lib [" OpenObscure Library "]
            direction TB
            S["<b>sanitizeText()</b><br/>Scan → FPE Encrypt"]
            R["<b>restoreText()</b><br/>FPE Decrypt → Firewall"]
            I["<b>sanitizeImage()</b><br/>NSFW → Face → OCR"]
        end
    ...
    style Lib fill:#e6f3f7,stroke:#545b64,stroke-dasharray: 5 5,color:#232F3E
    style A fill:#3b48cc,stroke:#232F3E,color:#fff
    style B fill:#ffffff,stroke:#545b64,color:#232F3E
    style S fill:#545b64,stroke:#232F3E,color:#fff
    style R fill:#545b64,stroke:#232F3E,color:#fff
    style I fill:#545b64,stroke:#232F3E,color:#fff
    style Cloud fill:#fff7ed,stroke:#ff9900,stroke-width:2px,color:#232F3E
    style LLM fill:#ff9900,stroke:#232F3E,stroke-width:2px,color:#fff
```
...
- [Request Flow — Gateway Model](#request-flow--gateway-model)
- [API Flow — Embedded Model](#api-flow--embedded-model)
...
- [Content-Type Handling (Gateway Model)](#content-type-handling-gateway-model)
- [PII Statistics Logging](#pii-statistics-logging)
- [Error Handling & Fail Mode](#error-handling--fail-mode)
- [Provider Routing (Gateway Model)](#provider-routing-gateway-model)
...
## API Flow — Embedded Model

No HTTP server. The app calls the library directly before sending to the LLM and after receiving the response. All detection and encryption logic is identical to the Gateway model.

```
sanitizeText(text: String) → SanitizeResult
        │
1. Hybrid scanner: regex → keywords → NER/CRF (same pipeline as Gateway Pass 2)
        │
2. For each PiiMatch: FF1 FPE encrypt → store mapping (keyed by request UUID)
        │
3. Return { sanitized_text, request_id, pii_count, scan_stats }
        │  (app sends sanitized_text to LLM; retains request_id for restore)

─────────────────────────────────────────────────────────────────

restoreText(sanitized_response: String, request_id: String) → RestoreResult
        │
1. String-replace FPE ciphertexts with original plaintext (using stored mapping)
        │
2. Response integrity scan: R1 dictionary → R2 TinyBERT (if sensitivity warrants)
        │
3. Clear mapping for request_id
        │
4. Return { restored_text, risk_report }  (risk_report contains manipulation flags if any)

─────────────────────────────────────────────────────────────────

sanitizeImage(image_bytes: Vec<u8>) → Vec<u8>
        │
1. Decode image bytes
        │
2. Same image pipeline as Gateway Pass 1:
   NSFW check → face solid-fill → OCR solid-fill → EXIF strip → re-encode
        │
3. Return redacted image bytes

─────────────────────────────────────────────────────────────────

sanitizeAudioTranscript(transcript: String) → String
        │
1. Run hybrid scanner on speech-to-text transcript
        │
2. Redact PII spans (hash-token, not FPE — transcript is ephemeral)
        │
3. Return redacted transcript

─────────────────────────────────────────────────────────────────

Errors surface as Result<T, OoError> return values — no HTTP status codes.
Fail-open / fail-closed semantics are identical to the Gateway model.
```

> **Key difference from Gateway:** The app is responsible for the LLM HTTP call. The library only handles PII sanitization and response restoration — it never touches the network.
...
### Passthrough-First Design (Gateway Model)
...
### Embedded Model: No Passthrough Auth

The embedded library never handles LLM HTTP requests. The host app owns the network layer and sends its own API keys directly to the LLM provider — OpenObscure is not in that path. There are no headers to forward, no health endpoint, and no auth token. The FPE key is loaded via `OpenObscureMobile::init(key_hex)` at app startup.

### Health Endpoint Auth Token (Gateway Model)
...
## Content-Type Handling (Gateway Model)
...
> **Embedded Model:** Content-type is not relevant — the app passes raw strings to `sanitizeText()` and raw image bytes to `sanitizeImage()` directly.

## PII Statistics Logging

PII match counts are always tracked **without logging PII values**. How they surface differs by deployment model.

**Gateway Model** — logged as structured tracing events per HTTP request:
...
**Embedded Model** — returned in the `SanitizeResult` struct from `sanitizeText()`:

```swift
let result = try lib.sanitizeText(text: input)
// result.piiCount      → total matches
// result.scanStats     → per-type breakdown (optional, if stats enabled)
```

No log output by default on mobile — the host app decides how to surface stats to the user or telemetry pipeline.

## Error Handling & Fail Mode

Configurable via `fail_mode` in `openobscure.toml`. The fail-open / fail-closed semantics apply to **both deployment models** — the difference is how errors surface to the caller.

### Gateway Model

Errors surface as HTTP status codes. Governs two code paths: per-span FPE failures and whole-body processing errors.
...
Errors surface as `Result<T, OoError>` return values — no HTTP status codes. The same fail-open / fail-closed logic applies:

| Situation | Fail-open | Fail-closed |
|-----------|-----------|-------------|
| Per-span FPE error | Hash-token fallback (`EMAIL_a7f2`); original forwarded if radix too small | `[REDACTED:<type>]` substituted |
| Pipeline error (image, voice, text) | Original value returned; `OoError::Warn` logged | `Err(OoError::Fatal)` returned |
| Vault / key unavailable | `Err(OoError::KeyUnavailable)` — always blocks regardless of fail mode | Same |

## Provider Routing (Gateway Model)
...
> **Embedded Model:** No routing — the app calls the LLM directly. Provider selection is the app's responsibility.
...
|--------|-------------|----------|
| `openobscure-core` | `[[bin]]` | Gateway Model: standalone HTTP proxy |
| `libopenobscure_core.a` | `[lib]` staticlib | Embedded Model: iOS static library |
| `libopenobscure_core.so` | `[lib]` cdylib | Embedded Model: Android shared library |
| `libopenobscure_core` | `[lib]` lib | Rust tests + integration crate |
