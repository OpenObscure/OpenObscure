...
impl Vault {
    ...
    // ── Versioned Key Management (Phase 5: Key Rotation) ────────────────

    /// Get the current FPE key version number.
    ///
    /// Resolution order:
    /// 1. `OPENOBSCURE_KEY_VERSION` env var
    /// 2. `fpe-key-current` keychain entry
    /// 3. If unversioned `fpe-master-key` exists (or env var), treat as version 1
    /// 4. Error
    pub fn get_fpe_key_version(&self) -> Result<u32, VaultError> {
        // 1. Env var override
        if let Ok(v) = std::env::var("OPENOBSCURE_KEY_VERSION") {
            return v.trim().parse::<u32>().map_err(|e| {
                VaultError::EnvVar(format!("OPENOBSCURE_KEY_VERSION is not a valid u32: {}", e))
            });
        }
        ...
        // 3. Unversioned key exists → implicit v1
        if self.fpe_key_exists() {
            return Ok(1);
        }
    ...
    ///
    /// Resolution order:
    /// 1. `OPENOBSCURE_MASTER_KEY_V{version}` env var
    /// 2. `fpe-master-key-v{version}` keychain entry
    /// 3. For version 1 only: fall back to unversioned key (migration path)
    pub fn get_fpe_key_by_version(&self, version: u32) -> Result<[u8; 32], VaultError> {
        // 1. Versioned env var
        let env_name = format!("OPENOBSCURE_MASTER_KEY_V{}", version);
        if let Ok(hex_key) = std::env::var(&env_name) {
            return self.parse_hex_key(&hex_key);
        }

        // 2. Versioned keychain
        let entry_name = format!("fpe-master-key-v{}", version);
        if let Ok(entry) = Entry::new(&self.service, &entry_name) {
        ...
        // 3. Version 1 falls back to unversioned key (backward compat)
        if version == 1 {
            return self.get_fpe_key();
        }

        Err(VaultError::KeyNotFound(format!(
            "FPE key version {} not found. Checked {} env var and {} keychain entry.",
            version, env_name, entry_name
        )))
    }

    /// Store a new versioned FPE key in the OS keychain.
    pub fn store_fpe_key_versioned(&self, version: u32, key: &[u8; 32]) -> Result<(), VaultError> {
        let entry_name = format!("fpe-master-key-v{}", version);
        let entry = Entry::new(&self.service, &entry_name).map_err(VaultError::Keyring)?;
        entry.set_secret(key).map_err(VaultError::Keyring)?;

        // In headless mode, print the key
        if std::env::var("OPENOBSCURE_HEADLESS").is_ok() {
    ...
    /// Retrieve a provider API key from the keychain.
    pub fn get_api_key(&self, provider: &str) -> Result<String, VaultError> {
        let entry = Entry::new(&self.service, &format!("api-key-{}", provider))
            .map_err(VaultError::Keyring)?;
        entry.get_password().map_err(VaultError::Keyring)
    }

    /// Store a provider API key in the keychain.
    pub fn set_api_key(&self, provider: &str, key: &str) -> Result<(), VaultError> {
        let entry = Entry::new(&self.service, &format!("api-key-{}", provider))
            .map_err(VaultError::Keyring)?;
        entry.set_password(key).map_err(VaultError::Keyring)
    }
}
