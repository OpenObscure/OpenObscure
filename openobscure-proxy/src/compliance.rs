...
pub fn run(command: ComplianceCommands, config: &AppConfig) -> anyhow::Result<()> {
    ...
    match command {
        ComplianceCommands::Summary { verbose } => {
            cmd_summary(audit_path, config, verbose)
        }
...
fn generate_ropa_markdown(entries: &[AuditEntry], config: &AppConfig) -> String {
    ...
    md.push_str("- Per-record unique tweaks (UUID + JSON path hash) preventing frequency analysis\n");
...
fn generate_dpia_markdown(entries: &[AuditEntry], config: &AppConfig) -> String {
    ...
    md.push_str("- **Key management:** OS keychain with env var fallback for headless environments\n");
    ...
    md.push_str("- **Breach detection:** Anomaly scoring on audit log (Art. 33 notification support)\n");
    ...
    md.push_str(&format!("- Audit log entries analyzed: {}\n", entries.len()));
    md.push_str(&format!("- Total PII detections: {}\n", total_pii));
    md.push_str("\n---\n*This document was auto-generated by OpenObscure Compliance CLI.*\n");
...
pub fn format_cef_line(entry: &AuditEntry) -> String {
    let severity = entry
        .pii_total
        .map(|n| if n > 10 { 7 } else if n > 0 { 5 } else { 3 })
        .unwrap_or(1);
    format!(
...
"#
    ...
    fn test_aggregate_pii_types() {
        ...
        let top2: std::collections::HashSet<_> = types[..2].iter().map(|(k, _)| k.as_str()).collect();
    ...
    fn test_ropa_json_generation() {
        ...
        assert_eq!(json["title"], "Record of Processing Activities (GDPR Art. 30)");
