...
use openobscure_core::scanner::PiiScanner;
...
fn parse_pii_type(s: &str) -> Option<PiiType> {
    match s {
        ...
        "PersonName" => Some(PiiType::Person),
...
/// Types detected by NER model (not regex/keywords).
fn is_ner_type(t: &str) -> bool {
    matches!(t, "PersonName" | "Location" | "Organization")
}
...
fn ner_models_available() -> bool {
    is_real_model(std::path::Path::new("models/ner/model_int8.onnx"))
}

fn make_ner_scanner() -> HybridScanner {
    use openobscure_core::ner_scanner::{NerPool, NerScanner};
    let scanner = NerScanner::load(std::path::Path::new("models/ner"), 0.60).unwrap();
    let pool = NerPool::new(vec![scanner]);
    HybridScanner::new(true, Some(pool), None)
}
...
fn test_corpus_loads_and_is_non_empty() {
    ...
    assert!(
        corpus.len() >= 450,
        "Corpus should have >= 450 entries, got {}",
        corpus.len()
    );
}
...
// ---------------------------------------------------------------------------
// NER model-gated accuracy tests
// ---------------------------------------------------------------------------

#[test]
fn test_ner_recall() {
    if !ner_models_available() {
        eprintln!("Skipping test_ner_recall: NER models not available");
        return;
    }

    let corpus = load_corpus();
    let scanner = make_ner_scanner();
    ...
    for (idx, entry) in corpus.iter().enumerate() {
        ...
        let expected_ner: Vec<&ExpectedMatch> = entry
            .expected
            .iter()
            .filter(|e| is_ner_type(&e.pii_type))
            .collect();

        if expected_ner.is_empty() {
            continue;
        }

        let detected = scanner.scan_text(&entry.text);

        for exp in &expected_ner {
            total_expected += 1;
            *per_type_expected.entry(exp.pii_type.clone()).or_insert(0) += 1;

            // Relaxed matching: NER span boundaries may differ slightly from
            // corpus annotations, so use substring containment.
            let found = detected.iter().any(|d| {
                let expected_type = parse_pii_type(&exp.pii_type);
                expected_type.is_some_and(|t| d.pii_type == t)
                    && (d.raw_value.contains(&exp.value) || exp.value.contains(&d.raw_value))
            });
    ...
    for (pii_type, expected) in &per_type_expected {
        let found = per_type_found.get(pii_type).copied().unwrap_or(0);
        let r = found as f64 / *expected as f64;
        let threshold = match pii_type.as_str() {
            "PersonName" => 0.85,
            "Location" => 0.70,
            "Organization" => 0.70,
            _ => 0.70,
        ...
        assert!(
            r >= threshold,
            "NER {} recall {:.3} is below target {:.2}. Missed {}/{}.",
            pii_type,
            r,
            threshold,
            expected - found,
    ...
    assert!(
        recall >= 0.80,
        "Overall NER recall {:.3} is below target 0.80. Missed {}/{}.",
        recall,
        total_expected - total_found,
        total_expected
    );
...
#[test]
fn test_ner_precision() {
    if !ner_models_available() {
        eprintln!("Skipping test_ner_precision: NER models not available");
        return;
    }
    ...
    for (idx, entry) in corpus.iter().enumerate() {
        ...
        for d in &detected {
            // Only count NER-type detections
            if d.pii_type != PiiType::Person
                && d.pii_type != PiiType::Location
                && d.pii_type != PiiType::Organization
            {
                continue;
            }
            ...
            // Relaxed matching: substring containment
            let matches_expected = entry.expected.iter().any(|exp| {
                let expected_type = parse_pii_type(&exp.pii_type);
                expected_type.is_some_and(|t| d.pii_type == t)
                    && (d.raw_value.contains(&exp.value) || exp.value.contains(&d.raw_value))
    ...
    if !false_positives.is_empty() {
        eprintln!("\n=== NER False positives (first 20) ===");
        for (line, pii_type, value) in false_positives.iter().take(20) {
            eprintln!("  line {}: {} \"{}\"", line, pii_type, value);
        }
    }

    // Lower threshold: many "false positives" are valid NER detections
    // in corpus entries that aren't annotated for Location/Organization
    // (e.g., detecting "Berlin" as Location in a PersonName test entry).
    assert!(
        precision >= 0.70,
        "NER precision {:.3} is below target 0.70. {} FP out of {} detections.",
        precision,
        false_positives.len(),
        total_detected
    );
