...
sudo cp target/release/openobscure /usr/local/bin/
...
sudo cp install/systemd/openobscure.service /etc/systemd/system/

# 3. Enable and start
sudo systemctl daemon-reload
sudo systemctl enable openobscure
sudo systemctl start openobscure

# Check status
sudo systemctl status openobscure

# View logs
journalctl -u openobscure -f

# Stop
sudo systemctl stop openobscure
```
