...
#[command(
    name = "openobscure",
    version,
    about = "OpenObscure L0 Core — FF1 format-preserving encryption and PII detection for LLM API calls"
)]
struct Cli {
...
fn service_is_loaded() -> bool {
    ...
    #[cfg(target_os = "linux")]
    {
        std::process::Command::new("systemctl")
            .args(["--user", "is-active", "--quiet", "openobscure"])
            .status()
            .map(|s| s.success())
            .unwrap_or(false)
    }
...
fn service_load() -> anyhow::Result<()> {
    ...
    if !path.exists() {
        anyhow::bail!(
            "Service not installed. Run 'openobscure service install' first.\n  \
             Expected plist at: {}",
            path.display()
        );
    }
...
<dict>
    ...
    <integer>5</integer>
        ...
        eprintln!();
        eprintln!("Run 'openobscure service start' to enable auto-restart.");
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let unit_path = unit_dir.join("openobscure.service");
...
"#,
        ...
        eprintln!("Run 'openobscure service start' to enable.");
    }
...
fn service_uninstall() -> anyhow::Result<()> {
    ...
    #[cfg(target_os = "linux")]
    {
        let unit_path = dirs_next::config_dir()
            .unwrap_or_else(|| std::path::PathBuf::from("~/.config"))
            .join("systemd/user/openobscure.service");
        if unit_path.exists() {
...
fn service_status() -> anyhow::Result<()> {
    #[cfg(target_os = "macos")]
    {
        ...
        if !plist.exists() {
            eprintln!("[OpenObscure] Service not installed");
            eprintln!("  Run 'openobscure service install' to set up.");
            return Ok(());
        }
