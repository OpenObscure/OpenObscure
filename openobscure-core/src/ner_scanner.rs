...
mod tests {
    ...
    /// Skipped in CI where Git LFS pointers are not resolved.
    #[test]
    fn test_mock_model_inference() {
        ...
        let model_path = mock_dir.join("model_int8.onnx");
        if !model_path.exists() {
            eprintln!("Skipping mock model test: {} not found", mock_dir.display());
            return;
        }
        // Git LFS pointer files are ~130 bytes; real models are much larger.
        // Skip when LFS hasn't resolved the file (e.g. CI without LFS checkout).
        if let Ok(meta) = std::fs::metadata(&model_path) {
            if meta.len() < 1024 {
                eprintln!(
                    "Skipping mock model test: {} looks like a Git LFS pointer ({} bytes)",
                    model_path.display(),
                    meta.len()
                );
                return;
